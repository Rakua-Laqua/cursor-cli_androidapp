        ...
        Surface(modifier = Modifier.fillMaxSize()) {
            NavHost(navController = navController, startDestination = AppDestination.Machines.route) {
                ...
                composable(AppDestination.Chat.route) {
                    ChatPlaceholderScreen(uiState = uiState, onBack = { navController.popBackStack() })
                }
            }
        }
    }
...
private fun ChatPlaceholderScreen(
    ...
    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Text(AppDestination.Chat.name, style = MaterialTheme.typography.headlineSmall)
        Text("${AppDestination.Chat.unimplementedTaskId} 未実装")
        StatusBlock(uiState)
    }
}
