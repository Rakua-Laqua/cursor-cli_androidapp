...
- `android/` — Android ネイティブクライアント。v1.16.0 は Workspace / Session / メモリ内 Chat、Permission approval、手動 Diff、応答内ファイルリンクと read-only Viewer、in-process 通知、Chat header の動的 Model Picker と Model Visibility、valid Context 表示、structured breakdown の条件付き内訳、valid Session Cost の独立 Usage。QR カメラと TLS は未完です。
...
Phase 2 は TASK-200〜204（v1.3.0〜v1.7.1）で Gate B 通過。Phase 3 の TASK-300〜303 は v1.8.0〜v1.11.0 で完了（TASK-300 は Gate C 通過）。Phase 4 の TASK-400〜405 は v1.12.0〜v1.16.0 で完了（Session Cost は v1.16.0。Account Usage は公式安定 interface が無く dormant）。次は Phase 5 TASK-500（実機必須、現状は未着手）。QR カメラ、TLS、履歴永続化 / 再接続復元は未完です。詳細は `CHANGELOG.md` と `docs/implementation_status.md` です。
